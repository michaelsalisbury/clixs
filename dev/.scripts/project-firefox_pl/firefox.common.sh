...
function url_is_valid(){
	...
	echo ${FUNCNAME}: ${url} 1>&2
	nslookup -query=hinfo -fail -retry=1 -timeout=1 ${url} | sed "s/^/${FUNCNAME},nslookup: /" 1>&2
	curl --connect-timeout ${timeout} -k -o /dev/null --head --silent --write-out "${FUNCNAME},curl prode: %{http_code}\n" "${url}" 1>&2
}
