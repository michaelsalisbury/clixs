...
function color_test(){
	...
	echo
	chc +n red underline -- "Forground Color Test                                                                                   "
	...
	echo
	chc +n red underline -- "Background Color Test                                                                                  "
	for c in `seq 40 47`; do
		color=$(get_format_name $c)
		chc $c -- "${fill:0: -${#color}}" ${color}
		for i in 2 0 1 ; do
			intensity=$(get_format_name $i)
...
function chc(){
	local CODE KEY VALUE NO_NEWLINE ECHO_FORMAT ESC=$'\e' esc='\e'
	while [[ "${1//[^[:alpha:]-+]/_}" != +(--|) ]]; do
		case "$1" in
			+n)	NO_NEWLINE="";;
			-e)	ECHO_FORMAT=true;;
	...
	case "${SHELL:-bash}" in
		/bin/bash|bash)
			ECHO_FORMAT="${ECHO_FORMAT:+${esc}[${CODE}m}"
			CODE="${ESC}[${CODE}m"
			CLEAR="${ESC}[0m"
			echo ${NO_NEWLINE--n} ${ECHO_FORMAT}${CODE}"$@"${@:+${CLEAR}}
			;;
	esac
}
function EXPORT(){
