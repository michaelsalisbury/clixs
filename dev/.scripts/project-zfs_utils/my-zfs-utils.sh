...
__make_boot_fs(){
	...
	# make distro-version fs as required
	filesystem+="/${distro,,}-${version}"
	if ! is_zfs_filesystem "${filesystem}"; then
...
__chroot_fs () {
	...
	# get mountpoint
	local dev mnt typ arg mount_entry IFS=')'
	< <(< <(get_mount_entry "${filesystem}") __sed_format_mount_entry) read dev mnt typ arg mount_entry
	unset IFS
