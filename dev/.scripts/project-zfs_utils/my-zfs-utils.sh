...
__make_boot_fs(){
	...
	zfs create "${filesystem}"
	${FUNCNAME}_${distro,,} "${filesystem}" "${@:2}"
	__make_boot_fs_zfs_utils "${filesystem}"
}
__make_boot_fs_zfs_utils(){
	local filesystem=${1}
	...
	# get mountpoint
	local mountpoint
	< <(get_mountinfo_path_entries "${filesystem}") read x mountpoint x x x x x
...
__make_boot_fs_ubuntu(){
	...
	if which debootstrap &> /dev/null; then
		...
		echo please install debootstrap and then try again.
		zfs umount "${filesystem}"
