...
class OC_USER_PWAUTH extends OC_User_Backend implements OC_User_Interface {
	...
	public function checkPassword( $uid, $password ) {
		...
		$handle = popen($this->pwauth_bin_path, 'w');
			...
			if (!in_array($unix_user['uid'], $this->pwauth_uid_list)) {
				...
				#OC_User::createUser($uid, (string)($msg->generate(30)));
				#$randomString = (string)($msg->generate(30));
				OC_User_Database::createUser($uid, (string)($msg->generate(30)));
				...
				#);
				#OC_User::setDisplayName($uid, 'Full Name Here');
			}
			return $uid;
		}
                return false;
	...
	public function setPassword( $uid, $password ) {
		OC_Log::write('OC_USER_PWAUTH', 'setPassword called, user:' . (string)$uid . ' passwd:' . (string)$password,3);
	...
	public function userExists( $uid ){
		...
		OC_Log::write('OC_USER_PWAUTH',
			      'userExists called, user:' . (string)$uid . ' posix:' . (string)(implode('|', (array_values( $user )) )),
			      3);
		OC_Log::write('OC_USER_PWAUTH',
			      'userExists called, user:' . (string)$uid . ' posix:' . (string)(implode('|', (array_keys( $user )) )),
			      3);
		OC_Log::write('OC_USER_PWAUTH',
			      'userExists called, user:' . (string)$uid . ' posix:' . (string)$user['uid'],
			      3);
