...
function DO_API_SNAPSHOT(){
	...
	# take snapshot
		...
		local DATA=$(jq -c -M -s add \
				<(jq -R {name:.} <<< "${DROPLET_NAME} ${SNAPSHOT_NAME} ${SNAPSHOT_DATE}")\
				<(jq -R {type:.} <<< snapshot))
		DAS_JSON_POST=$(DO_API_POST droplets/${DROPLET_ID}/actions DATA 2>/dev/null | jq -c -M .)
	...
	while true; do
		...
		if JQ_ '(.images[]|select(.name==$SNAPSHOT_IMAGE_NAME_OLD)|true)//false' <<< "${!DAS_JSON_IMAGES_REF}" | `cat`; then
			if (( SNAPSHOT_IMAGE_NUM < SNAPSHOT_RETENTION )); then
				...
				local DAS_JSON_POST=$(DO_API_DELETE images/${SNAPSHOT_IMAGE_ID} 2>/dev/null | tee do_snapshot.json.delete)
