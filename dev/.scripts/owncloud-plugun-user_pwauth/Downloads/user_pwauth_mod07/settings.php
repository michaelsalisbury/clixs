...
//OC_Log::write('OC_USER_PWAUTH', 'LIB :: ' . \OCA\user_pwauth\lib\Configuration::getAppName(), 3);
...
if ($_POST) {
	...
	foreach((array_keys($config::DEFAULTS)) as $param){
		if (isset($_POST[$param]) && $config->getParam($param) != $_POST[$param] ) {
			OC_Log::write('OC_USER_PWAUTH', 'POST :: ' . $param, 3);
			$config->params[$param] = $_POST[$param];
			$config->dbPushParam($param);
		}
...
// fill template
//$tmpl = new OC_Template( \OCA\user_pwauth\lib\Configuration::getAppName(), 'settings');
$tmpl = new OC_Template( $config::getAppName(), 'settings');
//foreach((array_keys($config::DEFAULTS)) as $param) $tmpl->assign( $param, $config->params[$param] );
foreach((array_keys($config::DEFAULTS)) as $param) $tmpl->assign( $param, $config->getParam($param) );
//foreach((array_keys($config->params)) as $param) $tmpl->assign( $param, $config->params[$param] );
