...
function sync_lock_(){
	...
	case "$1" in
		...
		-T|--tail-log)		shift; tail -n ${1-10} "${lock_path_log}" 2>/dev/null;;
		-H|--head-log)		shift; head -n ${1-10} "${lock_path_log}" 2>/dev/null;;
		-v|--verify-log)	shift; grep -q -x -F "$*" "${lock_path_log}" 2>/dev/null;;
		...
		-t|--timed-out)	[ -e "${lock_path}" ] &&
					#echo "\($(date +%s) - ${lock_timeout}\) < $(<"${lock_path}")"
...
function var_to_associative(){
	...
	case "${1}" in						# delim
		-d|--default)		local IFS=$' \t\n';;	# delim; default
		-l|--new-line)		local IFS=$'\n';;	# delim; newline
		-m|--ms-charage-return) local IFS=$'\r\n';;	# delim; charage-return
	...
	for _ref in $*; do
		...
		# echo ${FUNCNAME} :: "_word  = $_word"
		# echo ${FUNCNAME} :: "_words = $_words"
		# echo ${FUNCNAME} :: "_val   = $_val"
		unset _cnt
