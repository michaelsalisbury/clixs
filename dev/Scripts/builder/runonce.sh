...
function show_help(){
        ...
        echo -d Reset step to 1 and delete log
...
function switches(){
        [ -z "$*" ] && return
        while getopts "hlrdi:s:" OPTION; do
                case $OPTION in
                        h)              show_help; list_functions; echo; exit 1;;
                        l)              show_help; list_functions; echo; exit 1;;
                        i)              step=$OPTARG; eval_function $step
                                        exit 1;;
                        s)              step=$OPTARG; show_function $step
                                        exit 1;;
                        r)              rset;;
                        d)              rset; rm /var/log/$scriptName; exit 1;;
...
function main(){
        switches $*
        while [ $step -le $(list_functions | wc -l) ]; do
                skip
                eval_function ${step}
                sleep 1
                next
...
function list_functions(){ cat "$scriptPath/$scriptName" | sed '/^exit 0$/,$d;/^function setup/!d' | sed "s/[^_]*_\([^(]*\).*/\1/;s/_/ /g" | cat -n; }
...
function eval_function(){  $(name_function $1) 2>&1 | tee -a "/var/log/$scriptName"; }
