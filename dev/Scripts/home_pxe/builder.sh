...
function retr_version(){
	retr_version_cnt=`ls -1 "$scriptFQFN.v"* 2> /dev/null | sed 's|.*v\([0-9]*\)$|\1|' | sort -n | tail -1`
	let retr_version_cnt+=1
	echo $retr_version_cnt
	unset ${!retr_version*}
}
function make_backup(){ cd "$scriptPath"; mesg 80 Make Backup :: `cp -v "$scriptName" "$scriptName.v$(retr_version)"`; }
...
function padl(){ [[ $1 =~ ^[0-9]+ ]] && { padl_width=$1 && shift; } || padl_width=100
		 ...
		 padl_line+=" $@"
		 echo "${padl_line:$(( ${#padl_line} - $padl_width ))}"
		 unset padl_line
		 unset padl_width
		 unset padl_pattern
}
