...
*
      ...
      IMPLICIT REAL*8(A-H,O-Z)
      COMMON /EAV/CCA(10),CCB(35)
      INTEGER ICBPTR(0:4), I
      DATA    (ICBPTR(I), I=0,4) / 1,6,14,23,31 /
*
             IF (L .LE. LP) THEN
                 L1 = L
                 L2 = LP
...
* 
      ...
      J3 = I+K+N 
      J4 = J+L+N 
      IF (2*MAX(I,J,M) .LE. J1  .AND.  MOD(J1,2) .EQ. 0   .AND. 
     :    2*MAX(K,L,M) .LE. J2  .AND.  MOD(J2,2) .EQ. 0   .AND. 
     :    2*MAX(I,K,N) .LE. J3  .AND.  MOD(J3,2) .EQ. 0   .AND. 
     :    2*MAX(J,L,N) .LE. J4  .AND.  MOD(J4,2) .EQ. 0 )  THEN 
          J1 = J1/2 
          J2 = J2/2 
          J3 = J3/2 
          J4 = J4/2 
...
* 
	  ...
	  CHARACTER*3 EL(5),COUPLE(9) 
	  CHARACTER   STR*66,CH1*1,CH3*3 
* 
*  FULL   :  4l+2 
...
* 
	    IF (N .NE. 1) THEN 
 		IF (N .GT. 9) THEN 
	          STR=STR(1:J-1)//'('//CH3(1:2)//')' 
	        ELSE 
	          STR=STR(1:J-1)//'('//CH3(2:2)//')' 
	        ENDIF 
	        J=INDEX(STR,' ') 
...
* 
	    IF ((I .NE. 1 .AND. N .NE. FULL) .OR. M .EQ. 1) THEN 
		CH3=COUPLE(M+I-1) 
		CH1 = CH3(2:2) 
		IF (CH1.GE.'a' .AND. CH1.LE.'z') 
     :			CH3(2:2) = CHAR(ICHAR(CH1)-CONST) 
...
*     SOLVE A SYSTEM OF LINEQR EQUATIONS A*X = 0
...
*     AT THIS POINT IT IS ASSUMED THAT L U FACTORIZATION HAS ALREADY
*     BEEN PERFORMED
...
* 
      DOUBLE PRECISION FUNCTION V(I,J,II,JJ,K) 
      IMPLICIT REAL*8(A-H,O-Z) 
      COMMON /PARAM/H,H1,H3,CH,EH,RHO,Z,TOL,NO,ND,NWF,NP,NCFG,IB,IC,ID
     :   ,D0,D1,D2,D3,D4,D5,D6,D8,D10,D12,D16,D30,FINE,NSCF,NCLOSD,RMASS
      CALL DYK(I,II,K) 
