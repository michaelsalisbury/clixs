...
*
*
* ********** LISTING OF COMMON BLOCKS **********
*
*
* === INFORM
*     IREAD - DEFINITION OF INPUT CHANNEL (CAPABLE OF BEING REWOUND)
*     IWRITE - DEFINITION OF LINE PRINTER OUTPUT CHANNEL
*     IOUT - DEFINITION OF CARD PUNCH OUTPUT CHANNEL
*
* === DEBUG
*     IBUG1  -  DEFINES DEBUG IN H0WTS PACKAGE.
*          = 0   -   ONLY FINAL RESULTS PRINTED.
*          > 0   -   FULL RESULTS PRINTED FOR DEBUGGING.
*     IBUG2  -  DEFINES DEBUG IN RKWTS PACKAGE.
*          = 0   -   ONLY FINAL RESULTS PRINTED.
*          > 0   -   FULL RESULTS PRINTED FOR DEBUGGING.
*     IBUG3  -  DEFINES DEBUG IN RECOUPLING COEFFICIENT PACKAGE.
*          = 1   -   INTERMEDIATE RESULTS PRINTED FOR DEBUGGING.
*      IBUG4 TO IBUG9 ARE NOT USED IN THIS PACKAGE
*
* === KRON
*     IDEL(I,J),I,J=1,NHDEL  -  KRONECKER DELTA
*
* === STATES
*     NCFG   -   NUMBER OF CONFIGURATIONS
*     NOCCSH(I),I=1,NCFG   -    FOR EACH CONFIGURATION, THE NUMBER OF
*               OCCUPIED SHELLS
*     NOCORB(J,I),J=1,NOCCSH(I),I=1,NCFG   -   FOR EACH CONFIGURATION
*               THE NUMBERS INDICATING WHICH SHELLS ARE OCCUPIED
*     NELCSH(J,I),J=1,NOCCSH(I),I=1,NCFG   -   FOR EACH CONFIGURATION,
*               THE NUMBER OF ELECTRONS IN EACH OCCUPIED SHELL
*     J1QNRD(J,K,I),J=1,2*NOCCSH(I)-1,K=1,3,I=1,NCFG   -   ANGULAR
*               MOMENTUM QUANTUM NUMBERS.   K=1  IS SENIORITY,  K=2  IS
*               (2L+1),  K=3  IS (2S+1).   J=1,NOCCSH(I)  IS THE SET OF
*               QUANTUM NUMBERS FOR EACH SHELL.  THE REMAINING
*               NOCCSH(I)-1 ARE QUANTUM NUMBERS RESULTING FROM COUPLING
*               OF THE SHELLS
*     MAXORB   -   HIGHEST VALUE OF NOCORB(J,I)
*     LJCOMP(I),I=1,MAXORB   -    L  VALUES OF THE ORBITALS
*
* === TERMS
*     NROWS  -  NUMBER OF ROWS OF TABLE OF TERMS, ACCORDING TO
*              S,S**2,P,P**2,P**3,D,...D**5, ETC.
*     ITAB(I),I=1,NROWS  -  NUMBER OF SETS OF 3 QUANTUM NUMBERS IN
*               EACH ROW
*     JTAB(I),I=1,NROWS  -  IF WE LIST ALL QUANTUM NUMBERS IN A SINGLE
*               LINE, THIS MEASURES NUMBER OF QUANTUM NUMBERS OCCURRING
*               BEFORE ROW  I
*     NTAB(J),J=1,(JTAB(I)+3*ITAB(I))  -  LIST OF THE QUANTUM NUMBERS OF
*               THE TERMS
*
* === FACT
*     GAM(I),I=1,100  - LOG OF FACTORIAL (OR GAMMA FUNCTION FOR INTEGER)
*
* === MEDEFN
*          THIS LISTS THE QUANTUM NUMBERS NEEDED IN A SINGLE MATRIX
*          ELEMENT (PSI/V/PSIP)
*     IHSH  -  NUMBER OF SHELLS OCCUPIED (INCLUDING DUMMIES) IN PSI
*               OR PSIP
*     NJ(I),LJ(I),I=1,IHSH  -  N,L VALUES OF THE OCCUPIED SHELLS
*     NOSH1(I),NOSH2(I),I=1,IHSH  -  NUMBER OF ORBITALS IN EACH SHELL,
*               FOR PSI AND PSIP RESPECTIVELY
*     J1QN1(J,K),J1QN2(J,K),J=1,2*IHSH-1,K=1,3  -   K=1 IS SENIORITY ,
*               K=2 IS 2L+1  ,  K=3  IS 2S+1   .   THE FIRST IHSH VALUES
*               OF  J  ARE THE QUANTUM NUMBERS OF THE SHELLS, THE
*               REMAINING IHSH-1 ARE THE QUANTUM NUMBERS OBTAINED BY
*               COUPLING THE SHELLS
*               J1QN1 CORRESPONDS TO PSI,   J1QN2  TO PSIP
*          NOTICE THE DIFFERENT DEFINITION OF THIS BLOCK IN MUMDAD
*          AND SETUP,
*
* === MVALUE
*     M1,M2,.....M20
*               M1=ISIG-IRHO,   M2=ISIGP-IRHOP,   M19=IRHO-IRHOP
*               M20=ISIG-ISIGP  .   ALL THE REST OF THE M-VALUES ARE
*               SIMPLY RELATED TO IHSH AND ARE AS DEFINED IN RKWTS
*
* === XATION
*     KD1,KD2  -  LIMITS ON  K  FOR DIRECT INTEGRALS
*     KE1,KE2  -  LIMITS ON  K  FOR EXCHANGE INTEGRALS
*     AMULT(K),K=KD1,KD2,2 . BMULT(K),K=KE1,KE2,2  -  THESE STORE THE
*               MULTIPLYING FACTORS OF THE DIRECT AND EXCHANGE RK
*              INTEGRALS
*     MULTD,MULTE  -  ARE ZERO IF THERE ARE NO DIRECT/EXCHANGE INTEGRALS
*               TO CALCULATE
*
* === NJLJ
*     NRHO,LRHO,NSIG,LSIG,NRHOP,LRHOP,NSIGP,LSIGP
*               N,L VALUES OF THE SHELLS RHO,SIG,RHOP,SIGP
*
* === INTERM
*          INTERMEDIATE COUPLING QUANTUM NUMBERS
*     J1BAR1(I,K),J1BAR2(I,K),J1TLD1(I,K),J1TLD2(I,K),I=1,IHSH,K=1,3
*               K=1 IS SENIORITY, K=2 IS (2L+1),  K=3 IS (2S+1)
*               J1BAR1 AND J1BAR2 DEFINE QUANTUM NUMBERS OF SPECTATOR
*               ELECTRONS IN EACH SHELLS OF PSI AND PSIP RESPECTIVELY
*               J1TLD1 AND J1TLD2 ARE DEFINED ONLY IF RHO=SIG (OR
*               RHOP=SIGP).  THEN THESE ARE QUANTUM NUMBERS OBTAINED BY
*               COUPLING ONE OF THE TWO INTERACTING ELECTRONS ONTO THE
*               SPECTATOR ELECTRONS OF THE APPROPRIATE SHELLS
*          NOTICE THE DIFFERENT DEFINITION IN MUMDAD AND SETUP
*
* === COUPLE
*          THIS BLOCK HOLDS THE INPUT DATA FOR NJSYM
*     MI -  LENGTH OF J1 ARRAY
*     MJ -  NUMBER OF J2 OR J3 ROWS, PLUS 1
*     J1(I),I=1,3*IHSH+4(7)  -  (2J+1) VALUES  -  SPIN(ANGULAR)
*     J2(I),J),J3(I,J),I=1,IHSH+1(2),J=1,3  -  DETERMINE COUPLING
*               SCHEMES.  I LIMIT IS IHSH+1 FOR SPIN, IHSH+2 FOR ANGULAR
*
* === HOLD
*          STORAGE ARRAYS FOR SPIN AND ANGULAR COUPLING SCHEMES
*          STORES J2 AND J3 ARRAYS ROW AFTER ROW
*     J2SPIN(I),J3SPIN(I),I=1,3*(IHSH+1)
*     J2ANG(I),J3ANG(I),I=1,3*(IHSH+2)
*
* === DIAGNL
*     IDIAG = 0   FOR OFF-DIAGONAL MATRIX ELEMENT
*           = 1   FOR DIAGONAL MATRIX ELEMENT
*
* === ENAV
*          OUTPUT OF INFORMATION FROM INTERACTION ENERGY ROUTINE
*     NINTS - NUMBER OF  FK  OR  GK INTEGRALS OTHER THAN  F0
*     KVALUE(I),I=1,NINTS  -  THE K-VALUES OF THOSE INTEGRALS
*     COEFCT(I),I=1,NINTS - THE COEFFICIENTS MULTIPLYING THOSE INTEGRALS
*
* === REMOVE
*          ICHOP(I),I=1,IHSH    -
*                    = 1  IF ALL INTERACTIONS INVOLVING SHELL  I  MAY BE
*                            TREATED PURELY BY AVERAGE ENERGY
*                    = 0   OTHERWISE
*
* === STORE
*          USED AS A STORAGE AREA WHEN  MEDEFN  IS MODIFIED
*
*
*
* ********** END OF LIST OF COMMON BLOCKS **********
*
*
*
*
      ...
      COMMON/INFORM/IREAD,IWRITE,IOUT,ISC1,ISC2,ISC3,IALL,JSC1,JSC2,JSC3
...
*
    2 FORMAT(///10X,'TTTTT   O    N   N  III    A    N   N'/
     :          10X,'  T    O O   NN  N   I    A A   NN  N'/
     :          10X,'  T   O   O  N N N   I   AAAAA  N N N  -  82'/
     :          10X,'  T    O O   N  NN   I   A   A  N  NN'/
     :          10X,'  T     0    N   N  III  A   A  N   N'/)
*
*     .. Dimension limits for NJSYM
*
      KFL2=18
      KFL3=20
      KFL4=60
...
*
      PRINT '($,A)', ' IBUG1,IBUG2,IBUG3 ? '
      READ(5,'(3I)') IBUG1,IBUG2,IBUG3
      WRITE(IWRITE,1) IBUG1,IBUG2,IBUG3
      PRINT '($,A)', ' FULL PRINT-OUT ? (Y/N) '
      IFULL = 0
      READ(5,'(A1)') ANS
...
*
      CALL CFGN1
*
      PRINT '(/$,A)', ' ALL INTERACTIONS ? (Y/N) '
      READ(5,'(A1)') ANS
      IF (ANS .EQ. 'N' .OR. ANS .EQ. 'n' ) THEN
          PRINT '($,A)', ' NEW, N-ZERO, FIRST-ORDER(0/1) ? '
          READ(5,'(3I)') NEW, NZERO, IFIRST
      END IF
      IF ( NEW .EQ. 0 ) NEW = NCFG
      IF ( NZERO .EQ. 0 ) NZERO = NCFG
*
      PRINT '($,A)', ' MCHF or BREIT format ? (0 or 1) '
      READ (5,'(I1)') IALL
*
...
*
     ...
     :   (IOVL(II),IOVR(II),IOVEP(II),II=1,NOVLPS)
      IF (IALL.EQ.0)
     :   WRITE(ISC2,203) 2.*AC2,IZERO,IAJCLD(J),IAJCMP(JSIG),JA,
     :                         IAJCLD(J),IAJCMP(JSIGP),JB,
     :                (IOVL(II),IOVR(II),IOVEP(II),II=1,NOVLPS)
   15 IF (NINTS .EQ. 0) GO TO 11
     ...
     :    (IOVL(II),IOVR(II),IOVEP(II),II=1,NOVLPS)
      IF (IALL.EQ.0)
     :   WRITE(ISC2,203) 2.*ZA,K,IAJCLD(J),IAJCMP(JSIG),JA,
     :                          IAJCMP(JSIGP),IAJCLD(J),JB,
     :           (IOVL(II),IOVR(II),IOVEP(II),II=1,NOVLPS)
   12 CONTINUE
   11 CONTINUE
   14 IOTEMP = IOVR(1)
...
*
     ...
     :   (IOVL(II),IOVR(II),IOVEP(II),II=1,NOVLPS)
      ...
      WRITE(ISC2,203) Y,IZERO,IAJCMP(JRHO),IAJCMP(JSIG),JA,
   ...
   22 IF (NINTS .EQ. 0) GO TO 24
     ...
     :    (IOVL(II),IOVR(II),IOVEP(II),II=1,NOVLPS)
      Y = ZA
      IF (IALL .EQ. 0) Y = 2.D0*Y
      WRITE(ISC2,203) Y,K,IAJCMP(JRHO),IAJCMP(JSIG),JA,
...
*
      ...
      COMMON/COUPLE/NJ1S,NJ23S,J1(60),J2(18,3),J3(18,3)
      COMMON/OVRLAP/MU,NU,MUP,NUP,NONORT,NOVLPS,IROWMU,IROWNU,ICOLMU,
     : ICOLNU,NORTH,IORDER,NCALLS,LMU,LNU,LMUP,LNUP,JMU,JNU,JMUP,JNUP,
     :     IORTH(435)
      NJ23ST=NJ23S
...
*
     ...
     :     AMULT1(9),AMULT2(9),BMULT1(9),BMULT2(9)
...
*
  160 IL=IDEL(IRHO,ISIG)
      ...
      QD=0.5D0
  901 XMULT=DSQRT(Q)*QD
      ADIRCT=DFLOAT(2-IL-IR+IL*IR)/DSQRT(DFLOAT(4*LSIG*LRHOP+2*LSIG+2*LR
     :HOP+1))
      IEXCHG=2-IL-IR
      AEXCHG=DFLOAT(IEXCHG)/DSQRT(DFLOAT(4*LSIG*LSIGP+2*LSIG+2*LSIGP+1))
  902 DO 13 J=1,IHSH
  ...
  931 N1=IOVEP(1)
      N2=IOVEP(2)
      N3=N1+N2
      XMULT=XMULT*DEXP((GAM(N3+1)-GAM(N1+1)-GAM(N2+1))/2.D0)
  930 IDELP=0
      IF(M1) 14,15,14
   14 K1=IRHO+1
      DO 16 J=K1,ISIG
...
*
  ...
  978 DO 979 K = 1,3
      IF (J1BAR1(MU,K) .NE. J1BAR2(MUP,K)) GO TO 939
  979 CONTINUE
  950 CALL NJSYM(J6C,J7C,J8C,JWC,K6,K7,K8,KW,RECUP)
      SPINDT(1)=RECUP
      GO TO 948
  947 SPINDT(1)=0.0D0
...
*
  ...
  954 CALL NJSYM(J6C,J7C,J8C,JWC,K6,K7,K8,KW,RECUP)
      SPINEX(IORDER)=RECUP
      GO TO 955
  953 SPINEX(IORDER)=0.D0
...
*
      ...
      CALL J23ANG(IRHO,ISIG,IRHOP,ISIGP,JANGDI)
      IF(DABS(SPINDT(1)).LT.1.0D-14) GO TO 911
      J1(NJ1S)=2*JK1-1
      CALL NJSYM(J6C,J7C,J8C,JWC,K6,K7,K8,KW,RECUP)
      IFIRST=1
      ANGDIR(1)=RECUP
...
*
      ...
      GO TO 1911
  932 AMULT2(JK1)=AMULT2(JK1)+ANGDIR(2)*BDIRCT(2)*(-1)**NKAP
*
*     MULTD=1 WHEN A DIRECT INTEGRAL COEFFICIENT HAS BEEN CALCULATED -
*     FOR USE, SEE PRNTWT
*
 1911 MULTD=1
      IF(IFIRST.NE.1) GO TO 912
      GO TO 921
  911 ANGDIR(1)=0.D0
  921 IF(NOVLPS.LT.2) GO TO 912
      IF(DABS(SPINDT(2)).LT.1.D-14) GO TO 920
      IFIRST=0
      I=4
      CALL MODJ23(I)
      CALL NJSYM(J6C,J7C,J8C,JWC,K6,K7,K8,KW,RECUP)
  ...
  912 IF(IBUG2.LT.1) GO TO 114
      IF(NOVLPS.LT.2) GO TO 913
      WRITE(IWRITE,311) ANGDIR(1),ANGDIR(2)
      GO TO 114
  913 WRITE(IWRITE,310) ANGDIR(1)
  114 CONTINUE
*
...
*
  122 DO 115 JK1=KE1,KE2,2
      IF(DABS(SPINEX(IORDER)).LT.1.D-14) GO TO 918
*
*  --- MODIFY J2 AND J3 ARRAYS TO CALCULATE EXCHANGE TERMS
*
      I=2
      CALL MODJ23(I)
      J1(NJ1S) = 2*JK1-1
      ...
      IFIRST=1
      ANGEX(IORDER)=RECUP
  924 BMULT1(JK1)=BMULT1(JK1)-ANGEX(1)*BEXCHG(1)
      GO TO 945
  944 BMULT2(JK1)=BMULT2(JK1)-ANGEX(2)*BEXCHG(2)*(-1)**NKAP
  945 MULTE=1
      IF(IFIRST.NE.1) GO TO 919
      GO TO 923
  918 ANGEX(IORDER)=0.D0
  923 IF(NOVLPS.LT.2) GO TO 919
      IF(DABS(SPINEX(IOTHER)).LT.1.D-14) GO TO 914
      IFIRST=0
      I=4
      CALL MODJ23(I)
      CALL NJSYM(J6C,J7C,J8C,JWC,K6,K7,K8,KW,RECUP)
      ANGEX(IOTHER)=RECUP
      GO TO 944
  914 ANGEX(IOTHER)=0.D0
  919 IF(IBUG2.LT.1) GO TO 115
      IF(NOVLPS.LT.2) GO TO 946
       WRITE(IWRITE,313) ANGEX(1),ANGEX(2)
      GO TO 115
  946 WRITE(IWRITE,312) ANGEX(1)
  115 CONTINUE
  154 CONTINUE
  153 CONTINUE
  152 CONTINUE
...
*
      SUBROUTINE H0WTS
      IMPLICIT REAL *8(A-H,O-Z)
      DIMENSION L6( 60),L7( 80),L8( 60),LW(6,20)
...
*
     ...
     :    WRITE(ISC3,316) -A/2.,IAJCMP(ISIG),JA,IAJCMP(ISIGP),JB
 302  CONTINUE
...
*
  ...
  925 MUMIN1=MIN0(MU,MUP)+1
      ...
      IDELP=IDELP+NBAR(J)*NBAR(MU)
  927 CONTINUE
   70 FACTR=(-1.D0)**IDELP*DSQRT(DFLOAT(NOSH1(ISIG)*NOSH2(ISIGP)))
      IF(NOVLPS.LT.2) GO TO 102
      IF(MU.NE.NU.AND.MUP.NE.NUP) GO TO 102
      N1=IOVEP(1)
      N2=IOVEP(2)
      N3=N1+N2
      FACTR=FACTR*DEXP((GAM(N3+1)-GAM(N1+1)-GAM(N2+1))/2.D0)
*
...
*
   ...
   51 CONTINUE
*
*   S:M OVER PARENTS OF ISIG AND ISIGP
*
      N1=NOSH1(ISIG)
      L1=LSIG+1
      K1=NTAB1(N1,L1)
...
*
  ...
  937 CONTINUE
   78 CALL NJSYM(L6C,L7C,L8C,LWC,L6,L7,L8,LW,RECUP)
      GO TO 81
   79 RECUP=0.D0
   81 Y=Y*RECUP
  ...
  938 CONTINUE
   ...
   82 CONTINUE
      ...
      CALL NJSYM(L6C,L7C,L8C,LWC,L6,L7,L8,LW,RECUP)
   86 Y2=Y2*RECUP
  ...
  110 IF (NOVLPS.EQ.0) GO TO 98
   ...
   98 IF (DABS(Y) .GT. 1.D-10 ) THEN
	 ...
	 IF (IALL.NE.0) YY = YY/2.
         WRITE(ISC3,316) YY,IAJCMP(JSIG),JA,IAJCMP(JSIGP),JB
       ENDIF
       GO TO 1000
   ...
   92 IF (DABS(Y) .GE. 1.D-10) THEN
	 ...
	 IF (IALL.NE.0) YY = YY/2.
         WRITE(ISC3,316) YY,IAJCMP(JSIG),JA,IAJCMP(JSIGP),JB,
     :                  IAJCMP(JMU),IAJCMP(JMUP),IOVEP(1),
     :                  IAJCMP(JNU),IAJCMP(JNUP),IOVEP(2)
         B1ELC(IBK+1) = Y
      ENDIF
	 ...
	 IF (IALL.NE.0) YY = YY/2.
         WRITE(ISC3,316) YY,IAJCMP(JSIG),JA,IAJCMP(JSIGP),JB,
     :                    IAJCMP(JMU),IAJCMP(JNUP),IOVEP(1),
     :                    IAJCMP(JNU),IAJCMP(JMUP),IOVEP(2)
         B1ELC(IBK+2) = Y2
...
*
   ...
   18 IROMU1=IROWMU+1
      ...
      J3(IROMU1,ICOLNU)=J3(IRONU1,ICOLNU)
...
*
      ...
      COMMON/INFORM/IREAD,IWRITE,IOUT,ISC1,ISC2,ISC3,IALL,JSC1,JSC2,JSC3
      COMMON/DEBUG/IBUG1,IBUG2,IBUG3,NBUG6,NBUG7,IFULL
      CHARACTER GDATA*32, LDATA*52
      CHARACTER*28 STR, STR1, STR2, STR3, STR4
      CHARACTER*22 OVERL(30)
      CHARACTER*3 EL1(30),EL2(30),EL3(30),EL4(30)
      CHARACTER*1 X,LP,RP,COMMA
      DIMENSION C(30),K(30),I(30),J(30),IOV(30)
  100 FORMAT(14X,'*',16X,' ')
  101 FORMAT(14X,'*',22X,' ')
  102 FORMAT(14X,'*',14X,' ')
  200  FORMAT(A32)
  201  FORMAT(F14.8,A1,I2,A1,2A3,I3,A1,2A3,I3,A1,A)
  202  FORMAT(A)
*
* --- TERMINATE F INTEGERAL LIST
*
      WRITE(IOUT,100)
      ENDFILE ISC1
      REWIND ISC1
      ...
      REWIND ISC3
*
* --- NOW COPY THE G INTEGRALS
*
10    READ(ISC1,200,END=11) GDATA
      WRITE(IOUT,200) GDATA
      GO TO 10
   11 WRITE(IOUT,100)
      REWIND ISC1
*
* --- READ THE R INTEGRALS, REMOVING MULTIPLE
*     OCCURRENCES OF THE SAME INTEGRAL IN A GIVEN
*     INTERACTION
*
      NP = 1
      READ(ISC2,201) C(NP),X,K(NP),LP,EL1(NP),EL2(NP),I(NP),
     :                     COMMA,EL3(NP),EL4(NP),J(NP),RP,
     :                     OVERL(NP)
      IF (OVERL(NP)(12:12) .EQ. '<') THEN
         IOV(NP) = 22
        ELSE IF (OVERL(NP)(1:1) .EQ. '<') THEN
         IOV(NP) = 11
        ELSE
         IOV(NP) = 0
      END IF
      IF ( X .EQ. '*') GO TO 30
      NP = NP+1
   20 READ(ISC2,201) C(NP),X,K(NP),LP,EL1(NP),EL2(NP),I(NP),
     :                      COMMA,EL3(NP),EL4(NP),J(NP),RP,
     :                     OVERL(NP)
      IF (OVERL(NP)(12:12) .EQ. '<') THEN
         IOV(NP) = 22
        ...
        ELSE
         IOV(NP) = 0
      END IF
      IF ( I(NP) .NE. I(1) .OR. J(NP) .NE. J(1) .OR. NP .EQ. 30) THEN
            WRITE(IOUT,201) (C(M),'R',K(M),'(',EL1(M),EL2(M),I(M),
     :                  ',',EL3(M),EL4(M),J(M),')',
     :                   OVERL(M)(1:IOV(M)), M=1,NP-1)
            IF ( X .EQ. '*' ) GO TO 30
            C(1) = C(NP)
            K(1) = K(NP)
            EL1(1)=EL1(NP)
         ...
         ELSE
            DO 21 M=1,NP-1
               IF ( K(NP) .NE. K(M) ) GO TO 21
               IF ( OVERL(NP) .NE. OVERL(M) ) GO TO 21
               STR = EL1(NP)//EL2(NP)//EL3(NP)//EL4(NP)
               STR1 = EL1(M)//EL2(M)//EL3(M)//EL4(M)
               STR2 = EL2(M)//EL1(M)//EL4(M)//EL3(M)
               IF (STR.EQ.STR1 .OR. STR.EQ.STR2 .OR. STR.EQ.STR3
     :                         .OR. STR.EQ.STR4) THEN
*
*                    INTEGRAL THE SAME AS INTEGRAL M
*
                     C(M) = C(M) + C(NP)
                     GO TO 22
                ENDIF
...
* --- COPY L INTEGRALS
*
   31 READ(ISC3,202,END=32) LDATA
      WRITE(IOUT,202) LDATA
      GO TO 31
   32 WRITE(IOUT,102)
      REWIND ISC3
      RETURN
...
*
   ...
   15 IF((DABS(A).GE.1.D-10.OR.DABS(X).GE.1.D-10) .AND. IFULL .NE. 0)
     :   WRITE(IWRITE,2)A,X,Y,K,IAJCMP(JRHO),IAJCMP(JSIG)
      ...
      IF(DABS(Y).LT.1.D-10) GO TO 11
      WRITE(IOUT,100) Y,K,IAJCMP(JRHO),JA,IAJCMP(JSIG),JB
      GO TO 11
   16 IF(DABS(A).GE.1.D-10 .AND. IFULL .NE. 0)
     :   WRITE(IWRITE,3) A,X,Y,K,IAJCMP(JRHO),IAJCMP(JRHOP)
      ...
      IF(DABS(Y).LT.1.D-10) GO TO 11
      IF (IALL.EQ.0 .AND. JA.NE.JB) Y=Y*2.D0
      WRITE(ISC1,101) Y,K,IAJCMP(JRHO),JA,IAJCMP(JRHOP),JB
      GO TO 11
   13 NOVLP1=NOVLPS+1
  ...
  213 IF(DABS(A).GE.1.D-10 .AND.IFULL.NE.0) WRITE(IWRITE,4) A,X,Y,K,
     : IAJCMP(JRHO),IAJCMP(JSIG),IAJCMP(JRHOP),IAJCMP(JSIGP)
      ...
      IF (DABS(Y) .GE. 1.D-10)  WRITE(ISC2,102) Y,K,IAJCMP(JRHO),
     :    IAJCMP(JSIG),JA,IAJCMP(JRHOP),IAJCMP(JSIGP),JB
      GO TO 11
  ...
  413 IF(DABS(A).GE.1.D-10 .AND.IFULL.NE.0) WRITE(IWRITE,204) A,X,Y,K,
     ...
     : ,IAJCMP(JMUP),IOVEP(2),IAJCMP(JMU),IAJCMP(JNUP),IOVEP(1)
      ...
      IF (DABS(Y2) .GE. 1.D-10) WRITE(ISC2,102) Y2,K,IAJCMP(JRHO),
     :   IAJCMP(JSIG),JA,IAJCMP(JRHOP),IAJCMP(JSIGP),JB,
     :   IAJCMP(JNU),IAJCMP(JMUP),IOVEP(2),
     :   IAJCMP(JMU),IAJCMP(JNUP),IOVEP(1)
   11 CONTINUE
...
*
   ...
   25 IF(DABS(B)+DABS(X) .GE. 1.D-10 .AND. IFULL .NE. 0)
     : WRITE(IWRITE,3) B,X,Y,K,IAJCMP(JRHO),IAJCMP(JSIG)
      ...
      IF (IALL.EQ.0 .AND. JA.NE.JB) Y = Y+Y
      IF (DABS(Y) .GE. 1.D-10)
     :  WRITE(ISC1,101) Y,K,IAJCMP(JRHO),JA,IAJCMP(JSIG),JB
      GO TO 21
   31 IF(DABS(B)+DABS(X) .GE. 1.D-10 .AND. IFULL .NE. 0)
     :  WRITE(IWRITE,3) B,X,Y,K,IAJCMP(JRHO),IAJCMP(JSIGP)
      ...
      IF (IALL.EQ.0 .AND. JA.NE.JB) Y = Y+Y
      IF (DABS(Y) .GE. 1.D-10)
     :  WRITE(ISC1,101) Y,K,IAJCMP(JRHO),JA,IAJCMP(JSIGP),JB
      GO TO 21
   23 NOVLP1=NOVLPS+1
  ...
  223 IF(DABS(B).GE.1.D-10 .AND. IFULL.NE.0) WRITE(IWRITE,4) B,X,Y,K,
     : IAJCMP(JRHO),IAJCMP(JSIG),IAJCMP(JSIGP),IAJCMP(JRHOP)
      ...
      IF (IALL.EQ.0 .AND. JA.NE.JB) Y = Y+Y
      IF (DABS(Y) .GE. 1.D-10)
     :   WRITE(ISC2,102) Y,K,IAJCMP(JRHO),IAJCMP(JSIG),JA,
  ...
  423 IF(DABS(B).GE.1.D-10.AND.IFULL.NE.0) WRITE(IWRITE,204) B,X,Y,K,
     ...
     : ,IAJCMP(JMUP),IOVEP(2),IAJCMP(JMU),IAJCMP(JNUP),IOVEP(1)
      IF ( IALL .NE. 0) Y2 = B2
      IF (IALL.EQ.0 .AND. JA.NE.JB) Y2 = Y2+Y2
      IF (DABS(Y2) .GE. 1.D-10)
     :   WRITE(ISC2,102) Y2,K,IAJCMP(JRHO),IAJCMP(JSIG),JA,
     :                       IAJCMP(JSIGP),IAJCMP(JRHOP),JB,
     :                       IAJCMP(JNU),IAJCMP(JMUP),IOVEP(2),
     :                       IAJCMP(JMU),IAJCMP(JNUP),IOVEP(1)
   21 CONTINUE
...
*
   ...
   11 J1(M13)=2*LRHO+1
      ...
      NJ1S=M17
      NJ23S=NJ23S+1
   12 IF(IBUG2-1) 13,16,16
*
*     PRINT-OUT OF VALUES IN NJSYM  IF IBUG3=1
*
   16 IF(IBUG3.NE.1) WRITE(IWRITE,15)(J1(J),J=1,NJ1S)
   13 RETURN
      END
*
*     ------------------------------------------------------------------
...
*
    ...
    4 IF (IFULL .NE. 0)
     :  WRITE(IWRITE,6)AC2,AC2,ZERO,IZERO,IAJCMP(JRHO),IAJCMP(JSIG)
      IF (IALL .NE. 0) WRITE(IOUT,100) AC2,IZERO,IAJCMP(JRHO),JA,
     :                 IAJCMP(JSIG),JB
     ...
     :   WRITE(IWRITE,11) ZA,ZA,ZERO,K,IAJCMP(JRHO),IAJCMP(JSIG)
      IF (IALL .NE. 0) WRITE(ISC1,101) ZA,K,IAJCMP(JRHO),JA,
     :                 IAJCMP(JSIG),JB
      GO TO 8
    9 IF (IFULL .NE. 0)
     :   WRITE(IWRITE,6) ZA,ZA,ZERO,K,IAJCMP(JRHO),IAJCMP(JSIG)
      IF (IALL .NE. 0) WRITE(IOUT,100) ZA,K,IAJCMP(JRHO),JA,
     :                 IAJCMP(JSIG),JB
    8 CONTINUE
