...
* 
      ...
      PARAMETER(IWRITE=6)
      IMPLICIT REAL*8(A-H,O-Z) 
      CHARACTER CONFIG*66, COUPLE*6, EL(*)*3 
      COMMON /PARAM/H,H1,H3,CH,EH,RHO,Z,TOL,NO,ND,NWF,MASS,NCFG,IB,IC,ID
     :   ,D0,D1,D2,D3,D4,D5,D6,D8,D10,D12,D16,D30,FINE,NSCF,NCLOSD,RMASS
      COMMON /RADIAL/R(NOD),RR(NOD),R2(NOD),P(NOD,NWFD),YK(NOD),
     :   YR(NOD),X(NOD),AZ(NWFD),L(NWFD),MAX(NWFD),N(NWFD)
...
3     LI = L(I) 
      C = 2*LI + 1 
      IF (ICASE .EQ. 2 .AND. I .EQ. NWF) THEN
        A1 = D2/(C*(LI+1)) 
        A2 = -A1/((C+D2)*(LI+1)) 
        A3 = -A2/((LI+2)*(LI+1)) 
      ELSE
...
*                                       i     j 
* 
      DOUBLE PRECISION FUNCTION QUADRX(I,J,KK) 
      PARAMETER (NOD=880,NWFD=30) 
      IMPLICIT REAL*8(A-H,O-Z) 
      COMMON /PARAM/H,H1,H3,CH,EH,RHO,Z,TOL,NO,ND,NWF,MASS,NCFG,IB,IC,ID
     :   ,D0,D1,D2,D3,D4,D5,D6,D8,D10,D12,D16,D30,FINE,NSCF,NCLOSD,RMASS
...
* 
     ...
     :   YR(NOD),X(NOD),AZ(NWFD),L(NWFD),MAX(NWFD),N(NWFD)
      ...
      ZR = Z*R(1)
      ALPHA = -(A1+B1)/(Q+2)
      BETA = (A1+B1)**2/(Q+2) - 2*(A2+A1*B1+B2)/(Q+3)

      D = P(1,I)*P(1,J)*R(1)**K*(((BETA*ZR+ALPHA)*ZR+D1)/(Q+1) + D5)
      DD = D0
...
* 
     ...
     :   YR(NOD),X(NOD),AZ(NWFD),L(NWFD),MAX(NWFD),N(NWFD)
      ...
      K = 2 - KK 
      IF (ICASE .EQ. 2 .AND. I .EQ. NWF) THEN
        CD = D1 + Z*R(1)*(L(I)-L(J))/((DEN+D1)*((L(I)+1)*(L(J)+1))) 
      ELSE
...
* 
* 
      DOUBLE PRECISION FUNCTION ASHIFT(I1,I2) 
      IMPLICIT REAL*8(A-H,O-Z) 
      PARAMETER(NOD=880,NWFD=30) 
      COMMON /PARAM/H,H1,H3,CH,EH,RHO,Z,TOL,NO,ND,NWF,MASS,NCFG,IB,IC,ID
     :   ,D0,D1,D2,D3,D4,D5,D6,D8,D10,D12,D16,D30,FINE,NSCF,NCLOSD,RMASS
...
* 
     ...
     :                 +COEFVK(IP)*(V(I1,I,I,I2,K-1)-V(I,I1,I2,I,K-1)) 
   20   CONTINUE 
   10 CONTINUE 
      END 
* 
